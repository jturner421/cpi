...
def _calculate_total_elapsed_time(complaint_date, transfer_date, ua_date, dismissal_date, full_fee_paid,
    ...
    if ua_date is not pd.NaT:
        if transfer_date is not pd.NaT:
    ...
    if ua_date is pd.NaT:
        if full_fee_paid:
            if partial_payment_date is not pd.NaT:
                total_elapsed_time = (partial_payment_date - complaint_date).days
            else:
                total_elapsed_time = 1
        elif transfer_date is not pd.NaT:
            if dismissal_date is not pd.NaT:
                total_elapsed_time = (dismissal_date - transfer_date).days
            else:
                total_elapsed_time = (terminated_date - transfer_date).days
        elif dismissal_date is not pd.NaT:
            total_elapsed_time = (dismissal_date - complaint_date).days
        elif terminated_date is not pd.NaT:
...
def main():
    ...
    df['dismissal_date_for_no_trust_fund_statement'] = pd.to_datetime(df['dismissal_date_for_no_trust_fund_statement'])

    # check data conditions
    df['trust_fund_received_date'] = df.apply(lambda x: _check_trust_fund_received_date(x['trust_fund_received_date'],
                                                                                        x['partial_payment_date']),
                                              axis=1)
